...
tf.app.flags.DEFINE_string('val_dir', './data/val/', 'the val data dir')
...
tf.app.flags.DEFINE_string('TrainLabelPath', '/home/data_normal/bi_train/yujiangyong/crnndata/labels0314.txt', 'num of gpus')
tf.app.flags.DEFINE_string('ValLabelPath', '/home/data_normal/bi_train/yujiangyong/crnndata/val_labels0314.txt', 'num of gpus')
    ...
    def __init__(self, data_dir):
        ...
        for root, sub_folder, file_list in os.walk(data_dir):
            for file_path in file_list:
                ...
                code = int(float(image_name.split('/')[-1].split('.')[0].split('_')[1]))
                trainorval = image_name.split('/')[-1].split('.')[0].split('_')[0]
                
                if trainorval == 'train':
					labeldict = self.get_dict(FLAGS.TrainLabelPath)
                elif trainorval == 'val':
					labeldict = self.get_dict(FLAGS.ValLabelPath)
                code = labeldict[code]
                code = [SPACE_INDEX if code == SPACE_TOKEN else encode_maps[c] for c in list(code)[0:-1]]
