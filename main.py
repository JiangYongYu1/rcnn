...
import cnn_lstm_ctc as cnn_lstm_otc_ocr
import utils
import helper
        ...
        with tf.Session(config=config) as sess:
            ...
            for cur_epoch in range(FLAGS.num_epochs):
                ...
                for cur_batch in range(num_batches_per_epoch):
                    ...
                    batch_inputs, batch_labels, batch_seq_len = \
                    ...
                    if step % FLAGS.validation_steps == 0:
                        ...
                        for j in xrange(num_batches_per_epoch_val):
                            ...
                            val_inputs, val_labels, val_seq_len = \
    ...
    with tf.Session(config=config) as sess:
        ...
        for curr_step in xrange(total_steps):
            ...
            for img in imgList[curr_step * FLAGS.batch_size: (curr_step + 1) * FLAGS.batch_size]:
                _, imgname = os.path.split(img)
                im = cv2.imread(img, 0).astype(np.float32) / 255.
                im = np.reshape(im, [FLAGS.image_height, FLAGS.image_width, FLAGS.image_channel])
                im_input_name.append(imgname)
            ...
            for isdais, item in enumerate(dense_decoded_code):
                expression = ''
                ...
                decoded_expression.append(im_input_name[isdais].split('.')[0] + ' ' + expression)
